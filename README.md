...
> Note: a plain static server shows the marketing page but **not** the blog —
> the blog is built by Jekyll. To preview the blog, use the Docker command below.

## Preview the blog locally

The "Discover Formentera" blog is generated by **Jekyll** (the same engine
GitHub Pages uses). You don't need Ruby installed — just Docker:

```powershell
docker compose up
```

Then open **http://localhost:4000/casa-trini/blog/**. The first run installs the
gems (a few minutes); later runs are instant. Article and layout edits reload
